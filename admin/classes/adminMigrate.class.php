...
class WPM2AWS_Migrate
{
    ...
    public function registerLicenceForm()
    {
        ...
        if ($_POST['wpm2aws_licence_key'] === '' || get_option('wpm2aws_licence_key') === WPM2AWS_TRIAL_LICENCE_KEY) {
            $message = 'Success!<br><br>You are registered for a trial';
        } else {
            $message = 'Success!<br><br>Licence Key - Registered';
        }

        set_transient('wpm2aws_admin_success_notice_' . get_current_user_id(), __($message, 'migrate-2-aws'));

        $this->saveTrialUserIamForm();
...
//            exit(wp_redirect(admin_url('/admin.php?page=wpm2aws')));
        ...
        if ($user) {
            ...
            $validLicenceType = get_option('wpm2aws_valid_licence_type');

            if (strtoupper($validLicenceType) === 'TRIAL') {
                $awsResourceName = WPM2AWS_PLUGIN_AWS_RESOURCE . '-' . strtolower($user);
            } else {
                $currentDate = date('d_m_y');
                $uniqueId = uniqid('', true);

                $awsResourceName = sprintf('%s-%s-%s', WPM2AWS_PRO_LICENCE_DEFAULT_BUCKET_PREFIX, $currentDate, $uniqueId);
            }

            $awsLightsailName = strtolower(get_option('wpm2aws-iam-user')) . '-' . WPM2AWS_PLUGIN_AWS_RESOURCE;

            if ($validLicenceType !== false && strtoupper($validLicenceType) === 'TRIAL') {
            ...
            $awsResourceName = sanitizeUserEmailToAwsBucketName($awsResourceName);
            ...
            if (get_option('wpm2aws-customer-type') === 'managed') {
                ...
                if ($bucket) {
					...
					*/
                    ...
                    wpm2awsAddUpdateOptions('wpm2aws-aws-lightsail-size', WPM2AWS_PLUGIN_AWS_LIGHTSAIL_DEFAULT_SIZE_NAME);
...
//        exit(wp_redirect(admin_url('/admin.php?page=wpm2aws')));
    ...
    /**
     * Run the AWS credentials validation on Trial User
     */
    public function saveTrialUserIamForm()
    {
        ...
        if ($user) {
            ...
            // Set The value for default bucket name
            wpm2awsAddUpdateOptions('wpm2aws-aws-s3-default-bucket-name', $awsResourceName);
            ...
            // Set the Admin Notice
            set_transient('wpm2aws_admin_success_notice_' . get_current_user_id(), __('Success!<br><br>Trial user details stored', 'migrate-2-aws'));
        } else {
            wpm2awsLogRAction('wpm2aws_validate_iam_user_error', "Could not validate Trial User's AWS credentials");
            set_transient('wpm2aws_admin_error_notice_' . get_current_user_id(), __('Error!<br><br>Could not validate Trial User\'s AWS credentials<br><br>Please Try Again', 'migrate-2-aws'));
        }
...
//        exit(wp_redirect(admin_url('/admin.php?page=wpm2aws')));
    ...
    public function saveS3Bucket()
    {
        ...
        if (wpm2awsValidateBucketName($_POST['wpm2aws_s3BucketName']) === false) {
        ...
        if ($bucket) {
            $user = get_option('wpm2aws_licence_email');
        ...
        if ($bucket) {
            ...
            set_transient('wpm2aws_admin_success_notice_' . get_current_user_id(), __('Success!<br><br>New Bucket Created<br><br>Bucket Name: ' . $bucketName, 'migrate-2-aws'));
        } else {
...
//        exit(wp_redirect(admin_url('/admin.php?page=wpm2aws')));
    ...
    public function registerSeahorseLicence($licenceDetails)
    {
        ...
        return wpm2aws_register_licence($licenceDetails);
    }
    ...
    public function createBucket($restricted = true)
    {
        ...
        try {
            ...
            wp_die('<strong>Bucket Error</strong><br><br>' . $exception->getMessage() . '<br><br>Return to <a href="' . admin_url('/admin.php?page=wpm2aws') . '">Plugin Page</a>');
        }
        ...
        return $apiRemote->createBucket($restricted);
    }
...
//        exit(wp_redirect(admin_url('/admin.php?page=wpm2aws')));
    ...
    {
        $options = $this->getCommonProcessSavedOptions();

        $licenceType = get_option('wpm2aws_valid_licence_type');
        $upperCaseLicenceType = \strtoupper($licenceType);
        if ($upperCaseLicenceType === 'TRIAL') {
            $trialOptions = $this->getTrialUserProcessSavedOptions();

            $options = \array_merge($options, $trialOptions);
        }
     ...
     * @return array
     */
    private function getCommonProcessSavedOptions()
    {
        $sslChecksOptions = $this->getSslChecksOptions();
        ...
        return \array_merge($processOptions, $sslChecksOptions);
    }

    /**
     * @return string[]
     */
    private function getSslChecksOptions()
    {
        return array(
            'wpm2aws-ssl-error-bypass',
            'wpm2aws-remote-inv-ssl-error',
            'wpm2aws-remote-v-ssl-inv-php-v-error',
            'wpm2aws-remote-v-ssl-inv-wpp-v-error',
            'wpm2aws-local-inv-ssl-v-cacert-error',
            'wpm2aws-local-v-ssl-v-cacert-error',
            'wpm2aws-local-v-ca-inv-php-v-error',
            'wpm2aws-local-v-ca-inv-wpp-v-error',
            'wpm2aws-show-http-disclaimer',
            'wpm2aws-local-inv-ssl-inv-cacert-error',
            'wpm2aws-accept-ssl-error',
        );
    }

    /**
     * Get an array of the wp_options that relate to the "trial user process" of the migration
     *
     * @return array
     */
    private function getTrialUserProcessSavedOptions()
    {
        return array(
            'wpm2aws-iamid',
            'wpm2aws-iampw',
    ...
    {
        $processOptions = $this->getCommonProcessSavedOptions();
