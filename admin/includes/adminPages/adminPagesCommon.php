...
class WPM2AWS_adminPagesCommon
{
    ...
    protected static function makeActionButtons()
    {
        ...
        // Ordering depending on whether the licence key is confirmed or not
        if ($licenceType !== false && $licenceType !== '') { // Is confirmed
            // Manage AWS Block
        ...
        // Enable the following sections if the licence key is confirmed
        if ($licenceType !== false && $licenceType !== '') {
            $disabledSectionClass = '';
        }
    ...
    protected static function createStagingOnAwsBlock($licenceType, $sectionClass, $sectionLink, $panelPosition)
    {
        ...
        if (\in_array($licenceType, $createStagingAllowedLicenceTypes) === true) {
            $sectionLink =  WPM2AWS_SEAHORSE_WEBSITE_URL . '/how-to-quickly-and-easily-create-staging-development-sites-self-manage/';
        ...
        // Add action button link if available
        if ($sectionLink !== false) {
            $html .= '<a ' . $target . ' class="wpm2aws-action-button" href="' . $sectionLink . '">';
        }
        ...
        $html .= '<p>'. esc_html(__('Click this option for our tutorial on how to create a direct copy (existing stack version) for launching staging/dev sites', 'migrate-2-aws')) .'</p>';
