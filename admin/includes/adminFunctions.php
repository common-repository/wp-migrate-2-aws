...
function wpm2awsValidateBucketName($bucketName)
{
    ...
    $strippedBucketName = preg_replace('/([a-z])|([0-9])|([.])|([-])/', '', $bucketName);

    if (strlen($strippedBucketName) > 0) {
        return false;
    }

    // Should not be an IP address
    if (filter_var($bucketName, FILTER_VALIDATE_IP) !== false) {
        return false;
    }
    ...
    // - Cannot contain two, adjacent dashes
...
function sanitizeUserEmailToAwsBucketName($unsanitizedString)
{
...
function wpm2aws_register_licence($licence)
{
    ...
    $valid = ('200' === $responseCode || 200 === $responseCode);

    wpm2awsAddUpdateOptions('wpm2aws_valid_licence', $valid);

    if ($valid === true) {
        $licenceKey = (!empty($licence['wpm2aws-licence-key']) ? $licence['wpm2aws-licence-key'] : WPM2AWS_TRIAL_LICENCE_KEY);
        wpm2awsAddUpdateOptions('wpm2aws_licence_key', $licenceKey);
