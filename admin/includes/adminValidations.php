...
function wpm2awsValidatePost($form)
{
    ...
    if ($form === 'register-licence-form') {
        $licenceEmail = $_POST['wpm2aws_licence_email'];

        if ($licenceEmail === null || $licenceEmail === '') {
            return false;
        }

        if (empty($_POST['wpm2aws_accept_ssl_error']) === false) {
            add_filter('https_ssl_verify', '__return_false');
            add_filter('https_local_ssl_verify', '__return_false');

            wpm2awsAddUpdateOptions('wpm2aws-ssl-error-bypass', true);
        }
