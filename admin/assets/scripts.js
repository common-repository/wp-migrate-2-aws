...
jQuery(document).ready(function($) {
    ...
    $(".wpm2aws-register-licence-field").find('form').on("submit", function() {
        let loadingOverlay = $(".wpm2aws_loading_overlay");
        if (typeof loadingOverlay !== 'undefined' && loadingOverlay !== null && loadingOverlay.length > 0) {
            $(loadingOverlay).show();
        }
        $("#wpm2aws-settings-section-register-trial").hide();
        $("#wpm2aws-settings-section-register-pro").hide();
    });

    $("#wpm2aws-settings-button-trial-settings").on("click", function() {
        $("#wpm2aws-settings-section-register-pro").hide();
        $("#wpm2aws-settings-section-register-trial").show();
    });

    $("#wpm2aws-settings-button-pro-settings").on("click", function() {
        $("#wpm2aws-settings-section-register-trial").hide();
        $("#wpm2aws-settings-section-register-pro").show();
    });
  ...
  $('#wpm2aws-get-dynamic-progress').on("click", function() {
    ...
    // Diable register button for SSL checkboxes

    function toggleRegisterButtonBasedOnCheckbox() {
        var checkboxExists = $('input[name="wpm2aws_accept_ssl_error"]').length > 0;
        var isChecked = $('input[name="wpm2aws_accept_ssl_error"]:checked').length > 0;
        var isCertError = $('.wp2aws-cacert-error').length > 0;
    
        if (isCertError) {
            $('#wpm2aws-settings-section-register-trial input[type="submit"], #wpm2aws-settings-section-register-pro input[type="submit"]').attr('disabled', true);
        } else if (checkboxExists && !isChecked) {
            $('#wpm2aws-settings-section-register-trial input[type="submit"], #wpm2aws-settings-section-register-pro input[type="submit"]').attr('disabled', true);
        } else if (checkboxExists && isChecked) {
            $('#wpm2aws-settings-section-register-trial input[type="submit"], #wpm2aws-settings-section-register-pro input[type="submit"]').removeAttr('disabled');
        } else {
            $('#wpm2aws-settings-section-register-trial input[type="submit"], #wpm2aws-settings-section-register-pro input[type="submit"]').removeAttr('disabled');
        }
    }
    
    toggleRegisterButtonBasedOnCheckbox();
    
    $('input[name="wpm2aws_accept_ssl_error"]').change(function() {
        toggleRegisterButtonBasedOnCheckbox();
    });
