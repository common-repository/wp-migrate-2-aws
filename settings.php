...
if (!class_exists('WPM2AWS_Settings')) {
    class WPM2AWS_Settings
    {
        ...
        public static function checkSettings()
        {
	        ...
	        /*
	        * Run before each 'session' to remove records if present | can be redeclared on Item 7
	        */
            self::resetCACertError();

            $isValidHttps = self::isValidHTTPS();

            if ($isValidHttps === false) {
                self::setInvalidHttpsMessages();
            } else {
                self::setValidHttpsMessages();
            }
         ...
         * @return void
         */
        private static function setInvalidHttpsMessages()
        {
             ...
             * Item 1: REMOTE
             * Condition 1: HTTPS
             * Condition 2: localhost
             * result: if not HTTPS && not localhost = show disclaimer
             */
            if ($isLocalHost === false) {
                self::setItem1Messages();

                return;
            }

            $isValidCACertPresent = self::isValidCACertPresent();

            /*
            * Item 4: LOCAL
            * Condition 1: non HTTPS
            * Condition 2: Localhost
            * Condition 3: Valid CA Cert
            * result: if not HTTPS, and localhost with valid CA Cert = show disclaimer
            */
            if ($isValidCACertPresent === true) {
                self::setItem4Messages();

                $isSystemPhpVersionCompatibleWithSeahorseApiRequirements = self::isSystemPhpVersionCompatibleWithSeahorseApiRequirements();
                $isInstalledWordPressVersionCompatibleWithSeahorseApiRequirements = self::isInstalledWordPressVersionCompatibleWithSeahorseApiRequirements();
                ...
                * Condition 2: Localhost
                * Condition 3: Valid CA Cert
                * Condition 4: PHP Version
                * result: if not HTTPS, and localhost with valid CA Cert but not required SH API PHP version = show disclaimer
                */
                if ($isSystemPhpVersionCompatibleWithSeahorseApiRequirements === false) {
                    self::setItem5Messages();
                }
                ...
                * Condition 2: Localhost
                * Condition 3: Valid CA Cert
                * Condition 4: WP Version
                * result: if not HTTPS, and localhost with valid CA Cert but not required SH API WP version = show disclaimer
                */
                if ($isInstalledWordPressVersionCompatibleWithSeahorseApiRequirements === false) {
                    self::setItem6Messages();
                }
            ...
            * Condition 1: non HTTPS
            * Condition 2: Localhost
            * Condition 3: Invalid CA Cert
            * result: if HTTP, localhost && invalid CA Cert = no progress allowed
            */
            self::setItem7Messages();
        }

        /**
         * @return void
         */
        private static function setValidHttpsMessages()
        {
            $isLocalHost = self::isLocalhost();
            ...
            if ($isLocalHost === true) {
                ...
                * Condition 2: Localhost
                * Condition 3: Valid CA Cert
                * result: if HTTPS, and localhost with valid CA Cert = no action required, log only
                */
                if ($isValidCACertPresent === true) {
                    self::setItem41Messages();
                }
            ...
            * Condition 1: HTTPS
            * Condition 2: Environ
            * Condition 3: PHP Version
            * result: if HTTPS, not localhost && not required SH API PHP version = show disclaimer
            */
            if ($isSystemPhpVersionCompatibleWithSeahorseApiRequirements === false) {
                self::setItem2Messages();
            }
            ...
            * Condition 1: HTTPS
            * Condition 2: Environ
            * Condition 3: WP Version
            * result: if HTTPS, not localhost && not required SH API WP version = show disclaimer
            */
            if ($isInstalledWordPressVersionCompatibleWithSeahorseApiRequirements === false) {
                self::setItem3Messages();
            }
        }

        private static function setItem1Messages() {
            wpm2awsAddUpdateOptions('wpm2aws-remote-inv-ssl-error', true);
            wpm2awsAddUpdateOptions('wpm2aws-show-http-disclaimer', true);
        }
    
        private static function setItem2Messages() {
            wpm2awsAddUpdateOptions('wpm2aws-remote-v-ssl-inv-php-v-error', true);
            wpm2awsAddUpdateOptions('wpm2aws-show-http-disclaimer', true);
        }
    
        private static function setItem3Messages() {
            wpm2awsAddUpdateOptions('wpm2aws-remote-v-ssl-inv-wpp-v-error', true);
            wpm2awsAddUpdateOptions('wpm2aws-show-http-disclaimer', true);
        }

        private static function setItem4Messages() {
            wpm2awsAddUpdateOptions('wpm2aws-local-inv-ssl-v-cacert-error', true);
            wpm2awsAddUpdateOptions('wpm2aws-show-http-disclaimer', true);
        }

        private static function setItem41Messages() {
            wpm2awsAddUpdateOptions('wpm2aws_local_v-ssl_v-cacert', true);
        }

        private static function setItem5Messages() {
            wpm2awsAddUpdateOptions('wpm2aws-local-v-ca-inv-php-v-error', true);
            wpm2awsAddUpdateOptions('wpm2aws-show-http-disclaimer', true);
        }

        private static function setItem6Messages() {
            wpm2awsAddUpdateOptions('wpm2aws-local-v-ca-inv-wpp-v-error', true);
            wpm2awsAddUpdateOptions('wpm2aws-show-http-disclaimer', true);
        }

        private static function setItem7Messages() {
            wpm2awsAddUpdateOptions('wpm2aws-local-inv-ssl-inv-cacert-error', true);
            wpm2awsAddUpdateOptions('wpm2aws-unable-to-proceed-error', true);
        }

        /**
         * @return bool
         */
        public static function isSystemPhpVersionCompatibleWithSeahorseApiRequirements()
        {
            $systemPhpVersion = self::getSystemPhpVersion();

            return version_compare($systemPhpVersion, SH_API_REQUIRED_MINIMUM_PHP_VERSION, '>=');
        }

        /**
         * @return string
         */
        public static function getSystemPhpVersion()
        {
            return phpversion();
        }
         ...
         * @return bool
         */
        public static function isInstalledWordPressVersionCompatibleWithSeahorseApiRequirements()
        {
            $wordPressVersion = self::getWordPressVersion();

            return version_compare($wordPressVersion, SH_API_REQUIRED_MINIMUM_WP_VERSION, '>=');
        }
        ...
        /*
         * Run before each 'session' to remove records | can be redeclared with Item 7
         */
        public static function resetCACertError()
        {
            ...
            if ($localInvSslInvCacertError !== false && $unableToProceedError !== false) {
                delete_option('wpm2aws-local-inv-ssl-inv-cacert-error');
                delete_option('wpm2aws-unable-to-proceed-error');
            }
        }
         ...
         * @return bool
         */
        public static function isValidCACertPresent()
        {
            return (bool)ini_get('curl.cainfo');
        }
         ...
         * @return bool
         */
	    public static function isValidHTTPS()
        {
        ...
        private function loadAdminPageFiles()
        {
            ...
            require_once WPM2AWS_PLUGIN_DIR . '/inc/MigrationFormTemplateFieldDto.php';
